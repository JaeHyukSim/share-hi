...
export default {
  ...
  methods: {
    onSignIn(googleUser) {
      ...
      axios.post(`https://localhost/api/login/social`, googleform, {})
        .then(res => {
    ...
    login() {
      if (this.enable_login) {
        axios.post(`https://localhost/api/login/basic`, this.form, {})
          .then(res => {
            if (res.data.message == 'SUCCESS') {
              localStorage.setItem('token', res.data.content.token)
              this.$store.dispatch("LOGIN", res.data.content.member)
    ...
    resendpassword() {
      this.dialog = false
      axios.post(`https://localhost/api/member/findPW`, {
